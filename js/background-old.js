...
class BasicFunctions{
                    ...
                    for(item in match_array){
                        if(match_array.hasOwnProperty(item)){
                            if(match_array[item]["type"] === tabItemType && match_counter === 0){
                                match_counter++;
                                console.log("Найдено соответствие! ", match_array[item]["name"], ";  ", match_array[item]["name"], ";  ", match_array[item]["name"], ";  ", match_array[item]["size"]);
                                current__item = match_array[item];
                                console.log(current__item);
                                
                                console.log("Find item:", match_array[item]["type"], match_array[item]["type"]);
                                
                                ItemsActions.actionsOnTypePage({
                                    "name": match_array[item]["name"],
                                    "type": match_array[item]["type"],
                                    "size": match_array[item]["size"],
                                    "colors": match_array[item]["color"]
                                },GLOBAL);
    ...
    startActions(GLOBAL) {
        if (!GLOBAL["LAP_FLAG"]) {  //  Вход.
            console.log("placeTobasket or createTab&");
            ...
            console.log("Lap glag is guiltu!");
                            ...
                            var search__size = setInterval(function () {    //  Если список размеров загружен.
                                if( (document.querySelector('select[name="size"]') !== null) || (document.querySelector('b[class="button in-cart"]') !== null)){
                                    ...
                                    if(document.querySelector('b[class="button in-cart"]') !== null){   //  Если предмет уже в корзине.
                                        ...
                                        // Раскоментить для дальнейших действия на странице checkout.
